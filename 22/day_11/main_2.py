...
def compute_lcm(values):
    ...
    for x in range(1, len(values)):
        lcm = floor((lcm * values[x]) / gcd(lcm, values[x]))
...
class Monkey:
    ...
    def inspect_items(self, monkeys, lcm):
        while len(self.items) != 0:
            ...
            self.compute_condition(self.apply_operation(self.items.pop(0)), monkeys, lcm)
    ...
    # Got a hint for this, even though not a direct answer
    # We don't have to keep the actual value, but to preserve the same divisibility properties
    # So, to determine a rule that preserves the divisibility of a number including all the given numbers
    #   we have to compute the least common multiple between all of those numbers
    def compute_condition(self, item, monkeys, lcm):
        if (item % self.test) == 0:
            throw_item(item % lcm, int(self.test_true), monkeys)
        else:
            throw_item(item % lcm, int(self.test_false), monkeys)
